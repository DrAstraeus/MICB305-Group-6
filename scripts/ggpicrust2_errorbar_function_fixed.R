pathway_errorbar_fixed = function (abundance, daa_results_df, Group, ko_to_kegg = FALSE, 
  ...
  bar_errorbar <- ggplot2::ggplot(error_bar_pivot_longer_tibble_summarised_ordered, 
    ...
    GGally::geom_stripped_cols(width = 10) + ggplot2::scale_fill_manual(values = c("Supplement" = "#AAD9F4", "No Supplement" = "#F2CF7F")) + 
    ggplot2::scale_color_manual(values = c("Supplement" = "#AAD9F4", "No Supplement" = "#F2CF7F")) + ggprism::theme_prism() + 
  ...
  p_values_bar <- daa_results_filtered_sub_df %>% 
    ...
    ggplot2::geom_bar(stat = "identity", color = "black", position = ggplot2::position_dodge(width = 0.8), width = 0.8) + 
    ggplot2::labs(y = "Log2 Fold Change", x = NULL) + GGally::geom_stripped_cols() + 
    ggplot2::scale_fill_manual(values = "#C06CAB") + ggplot2::scale_color_manual(values = "black") + 
